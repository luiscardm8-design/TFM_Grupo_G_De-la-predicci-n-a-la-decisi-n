# TFM – Grupo G  
## De la predicción a la decisión: Simulador de descuentos para maximizar margen

**Autores**:  
- Luis Carlos Deza Monge   
- Álvaro Hernández Rubio   

**Máster**: Data Science KSchool
**Curso**: 2025-2026  

---

## 📌 Descripción
Este repositorio contiene el código desarrollado para el Trabajo Fin de Máster.
El objetivo del proyecto es desarrollar un simulador de descuentos que permita
maximizar el margen a partir de modelos predictivos.

---

## 💻 Contenido del repositorio
- Código fuente del proyecto.
- Video presentacion del proyecto.
- Memoria del TFM.

---
## 🎥 Vídeo de la defensa
👉 https://drive.google.com/file/d/1HCyKhGtgUjOQCaqoiO49EbcBVKoGyNcI/view?usp=drive_link
 )

---

## 📄 Memoria
La memoria del TFM se entrega a través de la plataforma oficial.
